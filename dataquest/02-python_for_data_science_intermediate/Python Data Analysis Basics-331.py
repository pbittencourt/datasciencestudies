...
def artist_summary(name):
    if name in artist_freq:
        total = str(artist_freq[name])
    else:
        total = '0'
        
    template = "There are {total} artworks by {name} in the data set"
    output = template.format(name=name, total=total)
    print(output)
...
for gender, artworks in gender_freq.items():
    ...
    print(template.format(artworks, gender))